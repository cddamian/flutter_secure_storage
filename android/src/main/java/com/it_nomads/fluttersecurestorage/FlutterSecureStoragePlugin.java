...
public class FlutterSecureStoragePlugin implements MethodCallHandler, FlutterPlugin {
    ...
    @FunctionalInterface
    interface ExceptionableFunction<T> {
        T run() throws Exception;
    }


    @FunctionalInterface
    interface WrapperExceptionableFunction {
        <T> T run(ExceptionableFunction<T> exceptionableFunction) throws Exception;
    }
    ...
    class MethodRunner implements Runnable {
        ...
        public void run() {
            try {
                ...
                WrapperExceptionableFunction tryOperation = secureStorage.getResetOnError() ? new WrapperExceptionableFunction() {
                    @Override
                    public <T> T run(ExceptionableFunction<T> exceptionableFunction) throws Exception {
                        try {
